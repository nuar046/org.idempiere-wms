...
public final class WMRuleEngine {
	 ...
	 */
	public Collection<MStorageOnHand> getMStorage(MWMInOutBoundLine line , int WM_Area_Type_ID , int WM_Section_Type_ID)
	{		
		MWMStrategy strategy = applyDefinition(line , WM_Area_Type_ID, WM_Section_Type_ID);
	...
	{						
		...
		if (definition == null)
		{
			throw new AdempiereException("Can not find valid WM Definition");
		}
	 ...
	 */
	public static Collection <MStorageOnHand> getMStorages(MWMStrategy strategy,int M_Product_ID,int M_AttributeSetInstance_ID, BigDecimal qtyToDeliver,int WM_Area_Type_ID , int WM_Section_Type_ID)
	{
		ArrayList<MStorageOnHand> targetStorages = new ArrayList();
		WMRuleEngine engine = WMRuleEngine.get();
		for(MWMStrategyDetail detail : strategy.getDetail())
		{
			MWMRule rule = (MWMRule) detail.getWM_Rule();
			...
			Collection<MStorageOnHand> storages = implementation.getStorage(
					strategy.getCtx(), 
					strategy.getM_Warehouse_ID(),
					M_Product_ID,
