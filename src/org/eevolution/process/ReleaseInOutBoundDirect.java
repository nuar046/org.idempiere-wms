...
import org.idempiere.model.MPPMRP;
import org.idempiere.model.MPPOrder;
import org.idempiere.model.MPPProductBOM;
import org.idempiere.model.MPPProductPlanning;
 ...
 *  @version $Id: $
 */
public class ReleaseInOutBoundDirect extends SvrProcess
{	
	/** Record ID */
	...
	protected String doIt () throws Exception
	{
		...
		String SQL = "SELECT iobl.PickedQty, iobl.WM_InOutBoundLine_ID FROM WM_InOutBoundLine iobl INNER JOIN WM_InOutBound iob ON (iob.WM_InOutBound_ID=iobl.WM_InOutBound_ID) "+ 
				...
				"WHERE iob.IsSOTrx='Y' AND iob.DocStatus='DR' AND iobl.Processed='N' "+
				"AND iobl.IsActive = 'Y'";
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		MWMInOutBound bound  = null;
		try
		{
			...
			while (rs.next ())
			{		
				MWMInOutBoundLine boundline = new MWMInOutBoundLine (getCtx(), rs.getInt(2), null);
				bound = boundline.getParent();
			...
			if (order != null) boundline.setProcessed(true); //red1 InOutBoundLine not ready for Shipment
			seq ++;
		  }
			if (order != null) {
				bound.completeIt(); //red1 InOutBound not ready for Shipment
			}
		}
	...
	{				
		...
		Collection<MStorageOnHand> storages = engineRule.getMStorage(boundline, p_WM_Area_Type_ID, p_WM_Section_Type_ID);
		...
		{	
			...
			{	
				throw new AdempiereException("This Warehouse is not Transit");
			}
			...
			if(order == null)
			{
				...
				{	
					throw new AdempiereException("There are no Users for this Business Partner"+ bp.getName());						
				}
	...
	public void createMO(MWMInOutBoundLine boundline, MProduct product, BigDecimal qtySupply)
	{
		...
		{	
			...
			//	if(bom == null) - red1 remove condition due to dead code
			//	{
